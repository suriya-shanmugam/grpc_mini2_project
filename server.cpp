...
void ConvertToProto(const CollisionProxy* data_struct, GRPCollision*  proto_data) {
...
/*struct grpc_query_field{


}*/

QueryType get_match_condition(Condition condition){
...
CollisionField get_match_field(QueryFields field){

    if (field == QueryFields::BOROUGH){
        return CollisionField::BOROUGH;
    }else if (field == QueryFields::ZIP_CODE){
...
struct GrpcQuery {
    ...
    public:
        ...
        GrpcQuery(CollisionField f, QueryType t, Value v): field(f),type(t),val(v){}
...
Query get_construted_query (const QueryRequest* request){

    std::vector<GrpcQuery> queries;
        ...
        std::vector<GrpcQuery>::iterator it; 
...
public:
    ...
    grpc::Status GetCollisions(grpc::ServerContext* context, const QueryRequest* request, QueryResponse* response) override{
        ...
        CollisionManager collision_manager{"/home/suriya-018231499/sem2/cmpe273/mini2/collisionproject/275_mini1/temp2/MotorVehicleCollisionData_subset.csv"};
        
        
        
        
        Query query = get_construted_query(request);
        ...
        for (auto proxy : collisions ){
            ...
            GRPCollision* collision = response->add_collision();
            ConvertToProto(proxy, collision);
            counter--;
        }
